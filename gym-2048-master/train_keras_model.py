...
''''自行更改的model'''
def build_model1(board_size=4, board_layers=16, outputs=4, filters=64, residual_blocks=4):
    ...
    x2 = layers.Reshape((board_size, board_size, board_layers))(inputs)
    ...
    x2 = layers.Conv2D(filters=filters, kernel_size=(5, 5), padding='same')(x2)
    ...
    for i in range(residual_blocks):
        ...
        temp_x = layers.Conv2D(filters=128, kernel_size=(3, 3), padding='same')(x)
        temp_x = layers.BatchNormalization()(temp_x)
        temp_x = layers.Activation('relu')(temp_x)
        temp_x = layers.Conv2D(filters=128, kernel_size=(3, 3), padding='same')(temp_x)
        temp_x = layers.BatchNormalization()(temp_x)
    ...
    m = models.Model(inputs=inputs, outputs=predictions)
    m.summary()
    # Create model
...
def build_model(board_size=4, board_layers=16, outputs=4, filters=64, residual_blocks=4):
    ...
    # Initial convolutional block
    x = layers.Conv2D(filters=filters, kernel_size=(5, 5), padding='same')(x)
    x = layers.BatchNormalization()(x)
    x = layers.Activation('relu')(x)
